...
const runTimeMovie = [
  { runtime: 30, name: "Girls" },
  { runtime: null, name: "Shining Girls" },
  { runtime: null, name: "Derry Girls" },
  { runtime: 60, name: "Good Girls" },
  { runtime: null, name: "Paper Girls" },
  { runtime: 60, name: "Gilmore Girls" },
  { runtime: null, name: "Surfside Girls" },
  { runtime: 60, name: "Land Girls" },
  { runtime: 30, name: "Florida Girls" },
];



function runTime() {
  let filmoviSodabranimRunTime = [];
  let runTimeButton = parseInt(document.getElementById("runTimeButton").value);

  document.getElementById("filtriraniFilmovi").innerHTML = "";

  for (let brojac = 0; brojac < runTimeMovie.length; brojac++) {
    if (runTimeButton == runTimeMovie[brojac].runtime) {
      filmoviSodabranimRunTime.push(runTimeMovie[brojac]);
      document.getElementById(
        "filtriraniFilmovi"
      ).innerHTML += `<li>${runTimeMovie[brojac].runtime} - ${runTimeMovie[brojac].name}</li>`;
    }
  }
}

function filtrirajUcenikeForEachom() {
  let runTimeButton = parseInt(document.getElementById("runTimeButton").value);

  document.getElementById("filtriraniFilmov").innerHTML = "";

  runTimeMovie.forEach((clan, index) =>
    filtriraj(clan, runTimeButton, index)
  );
}
